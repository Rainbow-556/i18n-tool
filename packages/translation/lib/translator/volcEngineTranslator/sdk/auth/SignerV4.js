...
SignerV4.sign = function (serviceInfo, apiInfo) {
  ...
  const metaData = new MetaData('HMAC-SHA256', credentials.service, credentials.region, currTime);
...
SignerV4.getSigningStr = function (apiInfo, header, metaData) {
  ...
  const signingStr = [metaData.algorithm, metaData.date, metaData.getCredentialScope(), hashCanonicalRequest].join(
    '\n'
  );
  return signingStr;
};
