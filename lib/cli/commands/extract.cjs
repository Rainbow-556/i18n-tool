...
const { genI18nDirAndFile } = require('../../utils/file/genI18nDirAndFile.cjs');
const { traverseFiles } = require('../../utils/file/traverseFiles.cjs');
const { getStagingFiles } = require('../../utils/git/getStagingFiles.cjs');
const { getStagingFileContent } = require('../../utils/git/getStagingFileContent.cjs');
const { addToStaging } = require('../../utils/git/addToStaging.cjs');
...
const { shouldProcessFile } = require('../../utils/file/shouldProcessFile.cjs');
...
async function extract({ staged }) {
  ...
  async function fileCallback(absolutePath) {
    ...
    let content;
    if (staged) {
      const [err, contentStr] = getStagingFileContent(absolutePath);
      if (err) {
        chalkLog('error', 'getStagingFileContent()失败', err);
        process.exit(1);
      }
      content = contentStr;
    } else {
  ...
  if (staged) {
    const [getStagingFilesErr, stagedFileAbsolutePaths] = getStagingFiles();
    if (getStagingFilesErr) {
      chalkLog('error', 'getStagingFiles()失败', getStagingFilesErr);
      process.exit(1);
    }
  ...
  if (staged) {
    ...
    addToStaging(path.resolve(i18nToolConfig.i18nDir, 'index.js'));
  }
  ...
  if (stats.entryCount > 0) {
    ...
    if (staged) {
      ...
      const [err] = addToStaging(path.resolve(i18nToolConfig.i18nDir));
      if (err) {
        chalkLog('error', 'addToStaging()失败', err);
        process.exit(1);
      }
