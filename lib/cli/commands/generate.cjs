...
async function generate() {
  ...
  let hasUncalibratedEntry = false;
  for (const lang of Object.keys(langPack.uncalibrated)) {
    if (Object.keys(langPack.uncalibrated[lang]).length > 0) {
      hasUncalibratedEntry = true;
      break;
    }
  }
  if (!hasUncalibratedEntry) {
    chalkLog('info', '没有待校准的词条，无需生成');
    return;
  }
