...
async function extract({ staged }) {
  ...
  if (staged) {
    ...
    for (const absolutePath of stagedFileAbsolutePaths) {
      if (shouldProcessFile(absolutePath)) {
        ...
        chalkLog('info', `⚠️ 跳过: 不在提取词条的文件范围: ${absolutePath}`);
      }
    }
  } else {
  ...
  if (stats.entryCount > 0) {
    ...
    chalkLog('info', '📈 翻译量统计(翻译中文成目标语言)');
    chalkLog('data', JSON.stringify(countInfo, null, 2));
    chalkLog('info', `⏱️ 翻译总耗时${((Date.now() - translationStartTime) / 1000).toFixed(2)}秒`);
    ...
    if (staged) {
      ...
      if (Object.keys(failResults).length > 0) {
        ...
        chalkLog('error', JSON.stringify(failResults, null, 2));
        chalkLog('error', `❌ 词条提取失败`);
        // 中断commit
        process.exit(1);
      } else {
        chalkLog('success', `✅ 所有词条翻译成功`);
        addToStaged(path.resolve(i18nToolConfig.i18nDir));
  ...
  if (!staged) {
    ...
    if (deleteEntry) {
      ...
      chalkLog('success', `✅ 冗余词条清理成功`);
      chalkLog('info', `ℹ️ 已清理的词条如下:`);
      chalkLog('data', JSON.stringify(deleteEntry, null, 2));
    }
  }
