...
async function extract({ staged }) {
  ...
  if (staged) {
    ...
    for (const absolutePath of stagedFileAbsolutePaths) {
      if (shouldProcessFile(absolutePath)) {
        ...
        chalkLog('info', `不在提取词条的文件范围:`, absolutePath);
      }
    }
  } else {
  ...
  if (stats.entryCount > 0) {
    // chalkLog('info', '词条数据');
    // chalkLog('info', JSON.stringify(pendingTranslateEntry, null, 2));
    ...
    chalkLog('info', `翻译总耗时${((Date.now() - translationStartTime) / 1000).toFixed(2)}s`);
