...
function checkConfig() {
  ...
  if (!translator.customTranslator) {
    ...
    if (!['baidu', 'deepSeek'].includes(translator.name)) {
      chalkLog('error', 'i18n-tool.config.cjs translator.name 仅支持 baidu 或 deepSeek');
      process.exit(1);
    }
    ...
    if (translator.name === 'baidu' && (!translator.options.appId || !translator.options.secretKey)) {
      chalkLog('error', 'i18n-tool.config.cjs 未配置 translator.options.appId 和 translator.options.secretKey');
      process.exit(1);
    }
