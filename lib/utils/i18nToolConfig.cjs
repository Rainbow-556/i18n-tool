...
function checkConfig() {
  // todo 校验其他配置
  const { i18nDir, i18nAlias, originLang, targetLangs, runtimeTargetLangConfig, translator } = i18nToolConfig;
  if (!i18nDir) {
  ...
  if (!translator.customTranslator) {
    ...
    if (translator.name === 'baidu' && (!translator.options.appId || !translator.options.secretKey)) {
      chalkLog('error', 'i18n-tool.config.cjs 未配置 translator.options.appId 或 translator.options.secretKey');
      process.exit(1);
    }
    if (translator.name === 'deepSeek' && !translator.options.apiKey) {
      chalkLog('error', 'i18n-tool.config.cjs 未配置 translator.options.apiKey');
      process.exit(1);
    }
    if (translator.name === 'volcEngine' && (!translator.options.accessKeyId || !translator.options.secretAccessKey)) {
      chalkLog(
        'error',
        'i18n-tool.config.cjs 未配置 translator.options.accessKeyId 或 translator.options.secretAccessKey'
      );
      process.exit(1);
    }
  }
