...
const { formatEntryString } = require('../core/formatEntryString.cjs');
...
function transformCode({ buildTool, filePath, code }) {
  // todo 要求业务工程的.browserlistrc需要配置not dead，否则babel为了兼容老浏览器会把js代码的模板字符串转成''.concat()的形式，导致无法把模板字符串当成一个词条来处理
