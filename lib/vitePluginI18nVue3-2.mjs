    ...
    buildEnd() {
      ...
      if (Object.keys(missingEntries).length > 0) {
        if (compileMode === 'serve') {
          chalkLog('info', `以下词条未提取，将会在 git pre-commit 中自动提取`);
          chalkLog('info', JSON.stringify(missingEntries, null, 2));
        } else if (compileMode === 'build') {
          chalkLog('error', `构建失败，以下词条未提取，请执行 npx i18n-tool extract 命令`);
          chalkLog('error', JSON.stringify(missingEntries, null, 2));
          process.exit(1);
        }
      }
    }
