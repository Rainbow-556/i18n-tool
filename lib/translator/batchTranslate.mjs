...
async function batchTranslateInner({ originLang, targetLang, textInfos, translator }) {
  ...
  return new Promise(resolve => {
    ...
    function processChunk() {
      ...
      console.log(`开始处理第 ${chunkIndex}，chunk总数为 ${chunks.length}`);
...
// 示例如何使用该函数
export async function batchTranslate({ pendingTranslateTexts, translatorOptions }) {
  // 格式
    ...
    for (const textInfo of textInfos) {
      if (textInfo.content.length > translator.maxCharsPerReq) {
        failResults[lang].push({
          ...textInfo,
          errCode: '-100',
          errMsg: `文本长度超过翻译接口的最大限制${translator.maxCharsPerReq}`
        });
      } else {
